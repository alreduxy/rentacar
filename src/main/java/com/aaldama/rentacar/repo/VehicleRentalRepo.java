...
public interface VehicleRentalRepo extends JpaRepository<VehicleRental, Integer> {
    ...
    @Query(value = "SELECT " + "CONCAT(US.FIRST_NAME, ' ', US.LAST_NAME) AS FULL_NAME, " +
            ...
            "inner join MODELS M on M.MODEL_ID = V.MODEL_ID " +
            "inner join MANUFACTURERS MA on MA.MANUFACTURER_ID = M.MANUFACTURER_ID " +
            "inner join LOCATIONS L on L.LOCATION_ID = V.LOCATION_ID WHERE US.USER_ID = :user_id ORDER BY VR.VEHICLE_RENTAL_ID",
            nativeQuery = true)

    public List<Object[]> listarVehiculosRentadosPorUsuario(@Param("user_id") Long user_id);
}
